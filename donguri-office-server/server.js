...
function runClaude({prompt, system, model, mode, thinking, cwd}){
  return new Promise((resolve)=>{
    ...
    if(thinking === 'deep') env.MAX_THINKING_TOKENS = '16000';
...
// ---------- Codex CLI 실행 (실험적) ----------
function runCodex({prompt, system, mode, cwd}){
  return new Promise((resolve)=>{
    ...
    const args = ['exec', '--sandbox', sandbox, '--skip-git-repo-check', '--output-last-message', tmp, full];
    ...
    req.on('end', async ()=>{
      try{
        ...
        const r = p.engine === 'codex'
          ? await runCodex({prompt:p.prompt, system:p.system, mode:p.mode, cwd})
